...
class depthFusion_view_effects(gym.Env):
    ...
    def __init__(self,
     ...
     if self.bunny == True:
         self.depth_max = 0.88
     elif self.view_effects:
         self.depth_max = 1.0
         # self.depth_max = 0.9
         # self.depth_max = 0.95
     else:
         self.depth_max = 1.0
     print('depth max', self.depth_max)
     ...
     if self.resolution == -1:
         resolution = (320,240)
         self.camK_path = 'camera-intrinsics2.txt'
     else:
         self.camK_path = 'camera-intrinsics.txt'
     # start unreal env
     docker = False
     ...
     if self.batch12 == True:
         ...
         gt_dir = '/hdd/AIRSCAN/datasets/houses3k_gt_ply/house_BAT6_full/groundtruth/'
    ...
    def _step(self, action = 0):
        ...
        if self.resolution == -1:
            ...
            depth = depth_conversion(depth_pt, 160)
        else:
    ...
    def _reset(self, start_pose_rel = None):
       ...
       out_pcl_np = depth_fusion_mult(self.log_dir, cam_K_file = self.camK_path, first_frame_idx = 0, base_frame_idx=1000, num_frames = self.count_house_frames + 1, save_pcd = self.save_pcd, max_depth = self.depth_max, house_id=self.house_id)
