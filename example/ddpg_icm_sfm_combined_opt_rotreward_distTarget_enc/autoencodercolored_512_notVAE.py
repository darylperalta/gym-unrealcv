...
def create_state_encoder(state_shape, enc_shape, vae=False):
    # state -> phi
...
def build_encoder(channels = 1,img_rows = 120,img_cols = 160, action_size = 3,enc_shape = (288,)):
    ...
    encoder = create_state_encoder(img_shape, enc_shape, vae=False)
    print('state encoder')
...
def build_decoder(vae = False, enc_shape= (288,), channels = 1):
    ...
    if vae == True:
        ...
        x = Conv2DTranspose(128, (5,5), strides=(3,2), padding ='same')(x)
        x = BatchNormalization()(x)
        x = LeakyReLU(.2)(x)
        ...
        x = Conv2DTranspose(channels,(5,5),strides=(2,2),padding ='same')(x)
        outputs = Activation('sigmoid')(x)
...
def main():
    ...
    if train == True:
        ...
        if vae == True:
            ...
            if cont == True:
                autoencoder.load_weights(old_model)
            autoencoder.fit_generator(dataloader(batch_size,img_rows = img_rows,img_cols = img_cols), epochs=epochs, initial_epoch = init_epoch,steps_per_epoch=steps, shuffle=True, callbacks=[checkpointer,eval_check])
