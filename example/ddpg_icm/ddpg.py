...
class DDPG_icm:
    def __init__(self, outputs, memorySize, discountFactor,
                 learningRate_Critic, learningRate_Actor, target_update_rate,
                 img_rows, img_cols, img_channels, enc_shape=(288,), icm_lr =0.01):
        """
        Parameters:
            - outputs: output size
            - memorySize: size of the memory that will store each state
    ...
    def get_intrinsic_reward(self,observation, action, obs_new):
        return self.forward.get_intrinsic_reward(observation, action, obs_new)
    ...
    def learnOnMiniBatch(self, miniBatchSize):
        ...
        self.forward.train(state_batch, action_batch, newState_batch)
